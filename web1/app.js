...
function cleanterOrderPayload(o){
  ...
  (o.items||[]).forEach(i=>{
    ...
    if(i.note) content.push({type:"text",text:"  * "+String(i.note)});
  });
...
function browserOrderPrint(o){
  ...
  const items=(o.items||[]).map(i=>{const q=Number(i.qty)||0,p=Number(i.price)||0;return `<div style="padding:4px 0;border-bottom:1px dotted #777"><b>${esc(i.name||"")}${i.variant?` (${esc(i.variant)})`:""}</b><br>${q} x ${rupiah(p)} <b style="float:right">${rupiah(q*p)}</b>${i.note?`<div class="n">* ${esc(i.note)}</div>`:""}</div>`;}).join("");
  const delivery={pickup:"COD / Ambil Sendiri",direct:"Antar Langsung",courier:"Ojek Online / Kurir"}[o.deliveryMethod]||o.deliveryMethod||"-";
...
async function printReceipt(o){
  try{await cleanterPrint(cleanterReceiptPayload(o));return true;}
    ...
    const number=(o.orderNo||"").split("-").at(-1)||"?";
    const items=(o.items||[]).map(i=>`<div style="padding:4px 0;border-bottom:1px dotted #777"><b>${esc(i.name||"")}${i.variant?` (${esc(i.variant)})`:""}</b><div style="display:flex;justify-content:space-between">${Number(i.qty)||0} x ${rupiah(i.price)} <b>${rupiah((Number(i.qty)||0)*(Number(i.price)||0))}</b></div>${i.note?`<div class="n">* ${esc(i.note)}</div>`:""}</div>`).join("");
